...
public class XieZiAdapter6 extends BaseQuickAdapter<Common3Bean,BaseViewHolder> {


    public XieZiAdapter6(int layoutResId) {
        super(layoutResId);
    }
...
//        adapter.addData(item.getSelect5());
        try {
            ImageView imageVi = helper.getView(R.id.image);
            Glide.with(mContext).load(NetWorkUrl.IMAGEURL+item.getPhoto()).into(imageVi);
            helper.setText(R.id.title,item.getTitle()).
